    ...
    _.each(this.serverRoutes, (route, path) => {
      ...
      if (route.config) {
        ...
        if (route.config.validate && Object.keys(route.config.validate).length > 0) {
            ...
            route.config.validate.body = route.config.validate.payload // hapi compatibiluty
            ...
            methods = methods.concat((req,res, next) => {
                // validate request
                Joi.validate({headers: req.headers, params: req.params,query: req.query, body: req.body}, validation, (err, result)  =>{
                        if(err) {
