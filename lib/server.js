...
module.exports = {
  ...
  createServer(app) {
    ...
    this.letsencrypt = this.webConfig.letsencrypt
  ...
  start(app, server) {
    ...
    return new Promise((resolve, reject) => {
      if (this.letsencrypt && app.config.env === 'production') {
        const le = greenlock.create(this.letsencrypt)
        this._createHttpsServer(le.httpsOptions, le.middleware(server)).then(resolve).catch(reject)
      }
      else if (this.ssl) {
