...
module.exports = {
  ...
  createServer (app) {
    ...
    if (config.session && config.session.secret) {
      this.webConfig.middlewares.session = session({secret: config.session.secret, resave: true, saveUninitialized: false})
    }
    else {
        ...
        this.nativeServer = https.createServer(this.ssl, server)
          .listen(this.port, this.host, (err) => {
            ...
            if (this.redirectToHttps || this.portHttp) {
              http.createServer(server).listen(this.portHttp, this.host, (err) => {
                if (err) return reject(err)
                resolve()
              })
            }
