...
module.exports = {
  ...
  addErrorHandlers (app, server) {
    server.use((req, res, next) => {
      ...
      if (req.accepts('html')) {
        res.render('404', { url: req.url }, (err, html) => {
          if (err) {
            app.config.log.logger.error('Error sending page 404, maybe you don\'t have a 404.html file', err)
    ...
    server.use(function(error, req, res, next) {
      res.status(500)
      app.config.log.logger.error(error)
      // respond with html page
      if (req.accepts('html')) {
        res.render('500', { url: req.url, error: error }, (err, html) => {
          if (err) {
            app.config.log.logger.error('Error sending page 500, maybe you don\'t have a 500.html file', err)
